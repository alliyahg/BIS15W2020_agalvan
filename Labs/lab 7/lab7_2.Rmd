...
Save your finished homerange app from question 3 in part 1 as a stand alone `.R` script. What packages do you need to make sure to load at the top? Run the app by clicking the Run App button. Set the run method in the Run App drop down to "Run External". ![Run External](runexternal.png){width=300px}  
...
When working with any package on shiny ui appearance, it's important to know how the ui is laid out. The ui is laid out in a bootstrap grid pattern with 12 columns that can be subdivided into rows. The grid is contained in the `fluidPage()` of our current app. We won't use `fluidPage()` with `shinydashboard` but rather `dashboardPage()`. A shiny dashboard contains a minimum of a header, a sidebar, and a body. The `dashboardPage()` still contains the bootstrap grid.  
```{r}
library(shinydashboard)
...
Boxes are a nice way to group inputs and display outputs in `shinydashboard`. We can display our inputs and outputs in a box with `box()`. Boxes need to be contained in a `fluidRow()`. We will use just one row. Edit your app to conatin boxes in the ui.
```{r}
ui <- 
  dashboardPage(
  dashboardHeader(title = "Plot Iris App"),
...
1. Load `IvindoData_DryadVersion.csv` and use the function(s) of your choice to get an idea of the overall structure, including its dimensions, column names, variable classes, etc. As part of this, determine if NAs are present and how they are treated.
```{r}
...
```{r}
ui <- dashboardPage(
  dashboardHeader(title = "Relative Abundance vs. Distance from Villages"),
  dashboardSidebar(),
  dashboardBody(
  fluidRow(
  box(title = "Distance vs. Relative Abundance", width = 3,
  selectInput("x", "Select X Variable", choices = c("RA_Birds","RA_Monkeys","RA_Apes", "RA_Elephant", "RA_Rodent", "RA_Ungulate"), 
              selected = "RA_Monkeys"),
  ),
  box(title = "Distance vs. Relative Abundance", width = 7,
  plotOutput("plot", width = "600px", height = "500px")
  )
  )
  )
...
server <- function(input, output, session) { 
  ...
  output$plot <- renderPlot({
    ggplot(gabon, aes_string(x = "Distance", y = input$x, fill = input$x)) + geom_point() + theme_light(base_size = 18)
  })
