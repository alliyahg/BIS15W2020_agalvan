...
```{r}
lepto_search<- entrez_search(db="nuccore", term= "Calileptoneta[Organism]", retmax=10)
lepto_search
```

2. Use `entrez_fetch()` to return your database hits as a character vector.
```{r}
lepto_seqs<- entrez_fetch(db= "nuccore", id=lepto_search$ids, rettype = "fasta")
lepto_seqs
...
```{r}
write.dna(lepto_seqs, file ="lepto_fasta_1.fasta", format = "fasta", append = FALSE, nbcol = 6, colsep = " ", colw = 10)
```
